...
void initializeMenuButtons(buttonState *state, brogueButton buttons[5]) {
    ...
    if (rogue.playbackMode) {
        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  " Unpause (%sspace%s) ", goldTextEscape,
                    whiteTextEscape);
        } else {
        ...
        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  " Omniscience (%stab%s) ", goldTextEscape,
                    whiteTextEscape);
        } else {
        ...
        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  " Next Turn (%sl%s) ", goldTextEscape,
                    whiteTextEscape);
        } else {
        ...
        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  " Search (%ss%s) ", goldTextEscape,
                    whiteTextEscape);
        } else {
...
boolean getInputTextString(char *inputText,
    ...
    if (useDialogBox) {
        ...
        y = ROWS / 2 - 5;   // BT: shifted up 4 lines to accomodate iPad on-screen keyboard
        clearDisplayBuffer(dbuf);
